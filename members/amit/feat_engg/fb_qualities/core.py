...
class FBQuality:
    ...
    def _hz_to_band(self) -> None:
        num_bands = config.fft_length // 2 + 1
        f1, f2 = self._frequencies
        f1, f2 = min(num_bands, int(f1 / num_bands)), min(num_bands, int(f2 / num_bands))
        self._band = [f1, f2]

    def get_spectrograms(self, audio: np.ndarray,
                         spec: tf.Tensor = None) -> tf.Tensor:
        if spec is None:
...
class FBQualities:
    ...
    def get_ratios_for(self, audio: np.ndarray) -> Dict:
        assert self.is_built, "FBQualities must be built first"

        found_ratios = {}
        spec = get_spectrogram(audio) # var name required for eval below

        for name in self.names:
            ratio = eval(f"self.{name}.get_ratio(audio, spec)")
            found_ratios[name] = ratio
        return found_ratios
