...
import create_model_lin
import data_generator_lin
import config
...
def train(conf: Dict, dry: bool = False) -> Any:
    ...
    batch_size = 6
    data_gen = data_generator_lin.DataGenerator(conf, batch_size)
    print(data_gen.input_shapes)
    model = create_model_lin.create_model(conf, data_gen.input_shapes, False)
    ...
    _ = model.fit(
        ...
        callbacks=[
            ...
            tf.keras.callbacks.ModelCheckpoint(
                f"checkpoints/lin_{conf.get('model_name')}" + "_loss_{val_loss:.4f}.h5",
                monitor="val_loss", save_best_only=True, save_weights_only=False, verbose=True
            ),
            tf.keras.callbacks.CSVLogger(f"logs/lin_{conf.get('model_name')}.csv", separator=",", append=False)
        ],
        verbose=True
    )
