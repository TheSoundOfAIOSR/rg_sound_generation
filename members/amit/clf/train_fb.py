...
import create_model_fb
import data_generator_fb
import config
...
def train(conf: Dict, dry: bool = False) -> Any:
    batch_size = 6
    data_gen = data_generator_fb.DataGenerator(conf, batch_size)
    print(data_gen.input_shapes)
    model = create_model_fb.create_model(conf, data_gen.input_shapes, False)
