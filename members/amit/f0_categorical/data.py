...
def f0_decode(f0_categorical):
    classes = np.argmax(f0_categorical, axis=-1)
    output = [pitch_values[c] for c in classes]
...
def data_generator(complete_data_generator):
    while True:
        ...
        inputs, outputs = _data_generator(features)
        yield inputs, outputs


def data_generator_all_features(complete_data_generator):
    while True:
        features = next(complete_data_generator)
        inputs, outputs = _data_generator(features)
        yield inputs, outputs, features
